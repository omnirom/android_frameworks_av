...
void AwesomePlayer::createAudioPlayer_l()
{
    ...
    if(mAudioPlayer == NULL) {
        ALOGD("AudioPlayer created, Non-LPA mode mime %s duration %lld\n", mime, mDurationUs);
#endif
...
status_t AwesomePlayer::initAudioDecoder() {
    ...
    ALOGV("Tunnel Mime Type: %s, object alive = %d, mTunnelAliveAP = %d",\
            mime, (TunnelPlayer::mTunnelObjectsAlive), mTunnelAliveAP);

    bool sys_prop_enabled = !strcmp("true",tunnelDecode) || atoi(tunnelDecode);
    ALOGV("maxPossible tunnels = %d", TunnelPlayer::getTunnelObjectsAliveMax());
    ...
    if (mOffloadAudio) {
        ALOGD("createAudioPlayer: bypass OMX (offload)");
        mAudioSource = mAudioTrack;
    } else {
        ALOGD("createAudioPlayer: use OMX");
        mAudioSource = mOmxSource;
    }
