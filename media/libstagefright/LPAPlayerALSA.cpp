...
mObserver(observer) {
    ALOGD("LPAPlayer::LPAPlayer() ctor");
    mObjectsAlive++;
...
void LPAPlayer::setSource(const sp<MediaSource> &source) {
    CHECK(mSource == NULL);
    ALOGD("Setting source from LPA Player");
    mSource = source;
}
...
status_t LPAPlayer::start(bool sourceAlreadyStarted) {
    ...
    ALOGD("mAudiosink->open() mSampleRate %d, numOutputChannels %d, mChannelMask %d, flags %d",mSampleRate,
    ...
    mStarted = true;
    mAudioSink->start();
    ALOGD("Waking up decoder thread");
    mLock.lock();
    mDecoderCv.signal();
    mLock.unlock();
...
status_t LPAPlayer::seekTo(int64_t time_us) {
    ...
    ALOGD("In seekTo(), mSeekTimeUs %lld",mSeekTimeUs);
    ...
    if (mReachedEOS) {
        ...
        mReachedOutputEOS = false;
        if(mPaused == false) {
            ALOGD("Going to signal decoder thread since playback is already going on ");
            mDecoderCv.signal();
            ALOGD("Signalled extractor thread.");
        }
    }
    ALOGD("seek done.");
    return OK;
...
void LPAPlayer::pause(bool playPendingSamples) {
    ...
    if (!mPauseEventPending) {
        ALOGD("Posting an event for Pause timeout");
    ...
    if (mAudioSink.get() != NULL) {
        ALOGD("AudioSink pause");
        mAudioSink->pause();
    }
}

status_t LPAPlayer::resume() {
    ALOGD("resume: mPaused %d",mPaused);
    status_t err = NO_ERROR;
    Mutex::Autolock autoLock(mLock);
    if ( mPaused) {
        CHECK(mStarted);
        if(mPauseEventPending) {
            ALOGD("Resume(): Cancelling the puaseTimeout event");
            mPauseEventPending = false;
...
size_t LPAPlayer::AudioSinkCallback(
    ...
    if (buffer == NULL && size == AudioTrack::EVENT_UNDERRUN) {
        ...
        if(me->mReachedEOS == true) {
            ...
            ALOGD("postAudioEOS mSeeking %d", me->mSeeking);
            me->mObserver->postAudioEOS(0);
        }else {
            ALOGD("postAudioEOS ignored since %d", me->mSeeking);
        }
    }
    return 1;
}
...
void LPAPlayer::reset() {
    ...
    if (mInputBuffer != NULL) {
        ALOGD("AudioPlayer releasing input buffer.");
...
void LPAPlayer::decoderThreadEntry() {
    ...
    ALOGD("mDecoderThreadEntry wait for signal \n");
    if (!mStarted) {
        mDecoderCv.wait(mLock);
    }
    ALOGD("decoderThreadEntry ready to work \n");
    mLock.unlock();
    ...
    while (!mKillDecoderThread) {
        ...
        if (mReachedEOS || mPaused || !mIsAudioRouted) {
            ALOGD("Going to sleep before write since "
            ...
            ALOGD("Woke up from sleep before write since "
                  "mReachedEOS %d, mPaused %d, mIsAudioRouted %d",
                  mReachedEOS, mPaused, mIsAudioRouted);
            continue;
        }
        ...
        if(!mKillDecoderThread) {
            ...
            if(lPaused == true) {
                ...
                // resume is called
                ALOGD("Going to sleep in decodethreadiwrite since sink is paused");
                mDecoderMutex.lock();
                mDecoderCv.wait(mDecoderMutex);
                ALOGD("Going to unlock n decodethreadwrite since sink "
            ...
            if(lSeeking == false && (mKillDecoderThread == false)){
                ...
                ALOGD("Fillbuffer before seeling flag %d", mSeeking);
                int lWrittenBytes = mAudioSink->write(local_buf, bytesWritten);
                ALOGD("Fillbuffer after write, written bytes %d and seek flag %d", lWrittenBytes, mSeeking);
            } else {
                ALOGD("Fillbuffer ignored since we seeked after fillBuffer was set %d", mSeeking);
            }
        }
    }
...
size_t LPAPlayer::fillBuffer(void *data, size_t size) {
    ...
    ALOGD("fillBuffer: Clearing seek flag in fill buffer");
    ...
    while (size_remaining > 0) {
...

        {
            ...
            if (mSeeking || mInternalSeeking) {
                ...
                mInternalSeeking = false;
                ALOGD("fillBuffer: Setting seek flag in fill buffer");
            }
        }
...
int64_t LPAPlayer::getMediaTimeUs() {
    Mutex::Autolock autoLock(mLock);
    ALOGD("getMediaTimeUs() mPaused %d mSeekTimeUs %lld mPauseTime %lld", mPaused, mSeekTimeUs, mPauseTime);
...
void LPAPlayer::onPauseTimeOut() {
    ...
    if(mSeeking == false) {
        ...
        ALOGD("do not update seek time %lld ", mSeekTimeUs);
    }
    ALOGD("newseek time = %lld ", mSeekTimeUs);
