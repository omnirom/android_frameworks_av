...
static AudioInput gAudioInput = AUDIO_IN_SUBMIX;
...
static status_t prepareAudioEncoder(sp<MediaCodec>* pCodec,
    ...
    if (gAudioInput == AUDIO_IN_SUBMIX) {
        // We make sure that the audio source is enabled
        err = android::AudioSystem::setDeviceConnectionState(
    ...
    sp<AudioSource> source = new android::AudioSource(
            gAudioInput == AUDIO_IN_SUBMIX ?
                AUDIO_SOURCE_REMOTE_SUBMIX : AUDIO_SOURCE_MIC,
            48000 /* sampleRate */,
            2 /* channelCount */);
    ...
    sp<MediaCodec> encoder = android::MediaCodec::CreateByType(looper, android::MEDIA_MIMETYPE_AUDIO_AAC, true);
    if (encoder == NULL) {
        fprintf(stderr, "ERROR: unable to create audio/mp4a codec instance\n");
        return -1;
    }
